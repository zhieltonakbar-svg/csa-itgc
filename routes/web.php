...
Route::middleware('auth')->group(function () {
    ...
    // IT Category Controls Navigation (Dashboard → view-only for Admin)
    Route::get('/it-category/{category}/controls', [DashboardController::class, 'showControls'])->name('dashboard.controls');

    // IT Category Management (Admin only — CRUD)
    Route::get('/it-categories', [\App\Http\Controllers\ItCategoryController::class, 'index'])->name('it-categories.index');
    Route::post('/it-categories', [\App\Http\Controllers\ItCategoryController::class, 'store'])->name('it-categories.store');
    Route::put('/it-categories/{it_category}', [\App\Http\Controllers\ItCategoryController::class, 'update'])->name('it-categories.update');
    Route::delete('/it-categories/{it_category}', [\App\Http\Controllers\ItCategoryController::class, 'destroy'])->name('it-categories.destroy');

    // IT RCM Management (Sidebar → Admin only, full capabilities)
    Route::get('/rcm', [DashboardController::class, 'rcmIndex'])->name('rcm.index');
    Route::get('/rcm/{category}/controls', [DashboardController::class, 'showRcmControls'])->name('rcm.controls');
    ...
    Route::post('/controls/{control}/transition', [\App\Http\Controllers\ControlController::class, 'transition'])->name('controls.transition');
    Route::get('/controls/{control}/evidence', [\App\Http\Controllers\ControlController::class, 'getEvidences'])->name('controls.evidences');
    Route::get('/controls/{control}/berita-acara', [\App\Http\Controllers\ControlController::class, 'downloadBeritaAcara'])->name('controls.beritaAcara');
    Route::delete('/controls/delete-all', [\App\Http\Controllers\ControlController::class, 'destroyAll'])->name('controls.destroyAll');
    Route::delete('/controls/delete-period', [\App\Http\Controllers\ControlController::class, 'destroyPeriod'])->name('controls.destroyPeriod');
    ...
    // UPTI Management (Admin only)
    Route::post('/uptis', [\App\Http\Controllers\UptiController::class, 'store'])->name('uptis.store');
    Route::put('/uptis/{upti}', [\App\Http\Controllers\UptiController::class, 'update'])->name('uptis.update');
    Route::delete('/uptis/{upti}', [\App\Http\Controllers\UptiController::class, 'destroy'])->name('uptis.destroy');
});
