...
/**
 * Sets up the new UPTI scope requested by the mentor:
 *   ITSM   -> Application "ITSM"          (UPTI already existed)
 *   ITPC   -> Application "Cybersecurity" (new UPTI)
 *
 * For EACH of these two UPTI/Application pairs, this seeder:
 *   1. Creates the UPTI if missing.
 *   2. Creates the Application, mapped to that UPTI.
 *   3. Creates an ApplicationPeriod for 2026 Q1 with all IT Categories.
 *   4. Creates 3 user accounts (Officer/Creator, Manager/Reviewer,
 *      Senior Manager/Approver), scoped to that UPTI.
 *   5. Creates the same 18 controls from "IT Risk & Control Matrix V1.0"
 *      (reused from the SAP seeder), all pre-marked Completed with
 *      reviewer/approver notes and Review Result set, so Berita Acara
 *      is immediately generatable for each — same pattern as
 *      SapItRcm2026Q1Seeder.
 *
 * Run with:
 *   php artisan db:seed --class=ItpcItsmScopeSeeder
 */
class ItpcItsmScopeSeeder extends Seeder
{
    /**
     * [upti_name, application_name, email_slug]
     */
    private array $scopes = [
        ['ITSM', 'ITSM', 'itsm'],
        ['ITPC', 'Cybersecurity', 'itpc'],
    ];
    ...
    public function run(): void
    {
        ...
        foreach ($this->scopes as [$uptiName, $appName, $slug]) {
            DB::transaction(function () use ($uptiName, $appName, $slug, $rows) {
                $this->seedOneScope($uptiName, $appName, $slug, $rows);
            });
        }

        $this->command->info('Done. Created/updated UPTI, Applications, users, and controls for ITSM and ITPC scopes.');
    }

    private function seedOneScope(string $uptiName, string $appName, string $slug, array $rows): void
    {
        // 1. UPTI
        $upti = Upti::firstOrCreate(['name' => $uptiName]);

        // 2. Application
        $application = Application::firstOrCreate(
            ['name' => $appName],
            ['description' => $appName, 'is_active' => true, 'upti_id' => $upti->id]
        );

        if (!$application->upti_id) {
            $application->update(['upti_id' => $upti->id]);
        }

        // Attach all IT Categories globally (legacy pivot, same as normal Add Application flow)
        ...
        // 3. ApplicationPeriod for 2026 Q1, with all categories attached
        $period = ApplicationPeriod::firstOrCreate([
            'application_id' => $application->id,
            'year' => 2026,
            'quarter' => 'q1',
        ]);
        $period->itCategories()->syncWithoutDetaching($allCategories->pluck('id'));

        // 4. Users — Officer/Manager/Senior Manager, scoped to this UPTI
        $accounts = [
            ["Officer ({$uptiName})", "officer.{$slug}@gmail.com", 'creator'],
            ["Manager ({$uptiName})", "manager.{$slug}@gmail.com", 'reviewer'],
            ["Senior Manager ({$uptiName})", "seniormanager.{$slug}@gmail.com", 'approver'],
        ];
        ...
        $this->command->info("[{$uptiName}] UPTI, Application \"{$appName}\", and 3 user accounts ready.");

        // 5. Wipe existing controls for this Application + 2026 Q1, then seed fresh
        ...
        $submittedAt = \Carbon\Carbon::create(2026, 2, 5, 9, 0);
        $reviewedAt  = \Carbon\Carbon::create(2026, 2, 12, 10, 0);
        $approvedAt  = \Carbon\Carbon::create(2026, 2, 18, 14, 0);
        ...
        $this->command->info("[{$uptiName}] Created {$created} Completed control(s) for \"{$appName}\" (2026 Q1).");
    }

    /**
     * Same 18 controls from IT Risk & Control Matrix V1.0.
     */
    private function controlRows(): array
    {
        return [
